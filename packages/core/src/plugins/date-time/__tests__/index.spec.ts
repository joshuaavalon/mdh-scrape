...
describe("Test dateTimePlugin", async () => {
  it("should equal", async () => {
    ...
    assert.equal(DateTime.fromISO("2024-01-01T00:00:00.00Z").toISO(), value.value?.toISO());
  });
  ...
  it("should span", async () => {
    ...
    assert.equal(DateTime.fromISO("2024-01-08T00:00:00.00Z").toISO(), value.value?.toISO());
  });
